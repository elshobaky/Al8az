...
var loghtml_temp = '<div class="act-time" num="%num%">' +                                 
                    ...
                    '</div>' +
                '</div>'
var msg_temp = 'لقد قمت ب%cat% <a href="%url%">%item_name%</a>'
var logData = {
            's'   : 0,
            'n'   : 20,
        };
...
$(document).ready(function() {
    ...
    $('#edit_profile').submit(function(event) {
        ...
        $('#edit_profile_end').append('<img src="/style/img/loader.gif" id="loading">');
        ...
        $.ajax(ajax_data)
            .done(function(data) {
                ...
                if (data.edit_profile===true) {
                    $('#edit_profile_end').append('<a class="btn btn-success" id="submit_state" href="/user/profile"><i class="icon_check_alt2" style="font-size:18px;"></i> تم الحفظ (إضغط لإعادة تحميل الصفحة)</a>');
    ...
    $('#edit_contact').submit(function(event) {
        ...
        $('#edit_contact_end').append('<img src="/style/img/loader.gif" id="loading">');
        ...
        var ajax_data = {
            type        : 'POST', 
            url         : '/user/profile/changecontact', 
            data        : formData,
            dataType    : 'json', 
            encode      : true
        }
        $.ajax(ajax_data)
            .done(function(data) {
                ...
                if (data.change_contact===true) {
                    $('#edit_contact_end').append('<a class="btn btn-success" id="submit_state" href="/user/profile"><i class="icon_check_alt2" style="font-size:18px;"></i> تم الحفظ (إضغط لإعادة تحميل الصفحة)</a>');
                    //window.location.reload();
                    }
                else {
                    if (data.error_f) {
